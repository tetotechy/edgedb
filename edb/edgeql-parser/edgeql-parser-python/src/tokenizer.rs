...
fn get_token_kind_and_name(
    ...
    let text = &token.text[..];
    match token.kind {
        Assign => (tokens.assign.clone_ref(py), tokens.assign_op.clone_ref(py)),
        SubAssign => (
        ...
        Arrow => (tokens.arrow.clone_ref(py), tokens.arrow_op.clone_ref(py)),
        Coalesce => (tokens.coalesce.clone_ref(py), tokens.coalesce.clone_ref(py)),
        Namespace => (
        ...
        Concat => (tokens.concat.clone_ref(py), tokens.concat.clone_ref(py)),
        GreaterEq => (tokens.op.clone_ref(py), tokens.greater_eq.clone_ref(py)),
        LessEq => (tokens.op.clone_ref(py), tokens.less_eq.clone_ref(py)),
        NotEq => (tokens.op.clone_ref(py), tokens.not_eq.clone_ref(py)),
        NotDistinctFrom => (
            tokens.op.clone_ref(py),
            tokens.not_distinct_from.clone_ref(py),
        ),
        DistinctFrom => (tokens.op.clone_ref(py), tokens.distinct_from.clone_ref(py)),
        Comma => (tokens.comma.clone_ref(py), tokens.comma.clone_ref(py)),
        OpenParen => (
