    ...
    async def sql_simple_query(
        ...
        try:
            ...
            if state is not None:
                ...
                self.write_sync(buf)
            for unit_i, unit in enumerate(query_units):
                if self.debug:
                    self.debug_print('SQL unit', unit)
                    self.debug_print(dict(self.prep_stmts.items()))
                if unit.frontend_only:
                    emulated_units.add(unit_i)
                    continue
                if (
                    unit.prepare is not None
                    and not self.before_prepare(
                        unit.prepare.be_stmt_name, dbver, buf
                    )
                ):
                    if self.debug:
                        self.debug_print('unit emulated')
                    emulated_units.add(unit_i)
                    continue
                if (
                    unit.deallocate is not None
                    and self.before_prepare(
                        unit.deallocate.be_stmt_name, dbver, buf
                    )
                ):
                ...
                if not stmt_name or self.before_prepare(stmt_name, dbver, buf):
                    if self.debug:
                        self.debug_print(f"sq pg msg >>'P'")
                ...
                if self.debug:
                    self.debug_print(f"sq pg msg >>'B'")
                ...
                if self.debug:
                    self.debug_print(f"sq pg msg >>'D'")
                ...
                if self.debug:
                    self.debug_print(f"sq pg msg >>'E'")
            ...
            if self.debug:
                self.debug_print(f"sq pg msg >>'C'")
            ...
            for unit_i, unit in enumerate(query_units):
                while not sync_received:
                    if unit_i in emulated_units:
                        dbv.on_success(unit)
                        fe_conn.on_success(unit)
                        ...
                        if not unit.injected:
                            # CommandComplete
                            msg_buf = WriteBuffer.new_message(b'C')
                            assert unit.command_tag, \
                                "emulated SQL unit has no command_tag"
                            msg_buf.write_bytestring(unit.command_tag)
                            buf.write_buffer(msg_buf.end_message())
                    ...
                    if self.debug:
                        self.debug_print(f"sq pg msg <<{chr(mtype)!r}")
                    ...
                    if mtype == b'1':  # ParseComplete
                        ...
                        if unit.prepare is not None:
                            be_stmt_name = unit.prepare.be_stmt_name
                            if self.debug:
                                self.debug_print(
                        ...
                        msg_buf = WriteBuffer.new_message(b'E')
                        if unit.prepare is not None:
                            translation_data = unit.prepare.translation_data
                            offset = -55  # len('PREPARE <stmt> AS (')'
                        else:
                            translation_data = unit.translation_data
                            offset = 0
                        while True:
                        ...
                        if unit.prepare is not None:
                            try:
                                del self.prep_stmts[unit.prepare.be_stmt_name]
                            except KeyError:
                                pass
                        fe_conn.on_error(unit)
                        dbv.on_error()
                        break
